    ...
    server: {
      ...
      proxy: {
        '/api': {
          ...
          changeOrigin: true,
          // FIX: The backend server is likely rejecting requests with a 'localhost' referer.
          // This sets the Referer header to match the target, bypassing this security check.
          configure: (proxy, _options) => {
            proxy.on('proxyReq', (proxyReq, req, res) => {
              proxyReq.setHeader('Referer', 'https://cps.safarnameh24.com/');
            });
          },
          // FIX: The '/api' prefix needs to be removed from the path before sending it to the target server.
